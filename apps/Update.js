...
export class update extends plugin {
  ...
  async runUpdate(isForce) {
    ...
    this.oldCommitId = await this.getcommitId('mj-plugin')
    uping = true
    const ret = await this.execSync(command)
    uping = false

    if (ret.error) {
      logger.mark(`${this.e.logFnc} 更新失败：mj-plugin`)
      this.gitErr(ret.error, ret.stdout)
      return false
    }

    /** 获取插件提交的最新时间 */
    const time = await this.getTime('mj-plugin')

    if (/(Already up[ -]to[ -]date|已经是最新的)/.test(ret.stdout)) {
      await this.reply(`mj-plugin已经是最新版本\n最后更新时间：${time}`)
    } else {
      await this.reply(`mj-plugin\n最后更新时间：${time}`)
      this.isUp = true
      /** 获取mj-plugin的更新日志 */
      const log = await this.getLog('mj-plugin')
  ...
  async getLog(plugin = '') {
    ...
    let end = ''
      ...
      '更多详细信息，请前往gitee查看\nhttps://github.com/ap-plugin/mj-plugin/commits/main'

    log = await this.makeForwardMsg(`mj-plugin更新日志，共${line}条`, log, end)
