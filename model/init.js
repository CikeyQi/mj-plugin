...
class Init {
  ...
  initPolicy() {
    ...
    let policy_default_path = `${pluginRoot}/config/policy_default.yaml`
    if (!fs.existsSync(policy_default_path)) {
      Log.e('默认设置文件不存在，请检查或重新安装插件')
      return true
    }
    // 判断config/config/policy.yaml是否存在
    let policy_path = `${pluginRoot}/config/config/policy.yaml`
    if (!fs.existsSync(policy_path)) {
      Log.e('设置文件policy.yaml不存在，将使用默认设置文件')
      fs.copyFileSync(policy_default_path, policy_path)
    }
    // 同步policy.yaml和policy_default.yaml
    let policy_default_yaml = Config.getDefPolicy()
    let policy_yaml = Config.getPolicy()
    for (let key in policy_default_yaml) {
      if (!(key in policy_yaml)) {
        policy_yaml[key] = policy_default_yaml[key]
      }
    }
    for (let key in policy_yaml) {
