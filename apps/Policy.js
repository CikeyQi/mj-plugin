...
export class Setting extends plugin {
  ...
  async setGroupCoolTime (e) {
    ...
    const [
      globalGroupCoolTime,
      globalPersonCoolTime,
      currentGroupCoolTime,
      currentPersonCoolTime,
      switchGroupCoolTime,
      switchPersonCoolTime
    ] = [
    ...
    // 未指定群则为当前群
    const groupID = e.msg.match(/全局/)
      ? 'global'
      : undefined || e.msg.match(/群聊(\d{5,11})?/)[1] || e.group_id
    // 获取时长,关闭全局CD时为1
    const CoolTime = isNaN(/(cd|冷却)(.*)/i.exec(e.msg)[2])
      ? undefined
      : /(cd|冷却)(.*)/i.exec(e.msg)[2] || 60
    // CD时长
    ...
    if (switchGroupCoolTime) {
      this.setGroupPolicy(
        groupID,
        'group_cool_time_switch',
        switchGroupCoolTime[4] === '开启'
      )
    }
    if (switchPersonCoolTime) {
      this.setGroupPolicy(
        groupID,
        'person_cool_time_switch',
        switchPersonCoolTime[4] === '开启'
      )
    }
    return true
  }
  ...
  async setPrivateCoolTime (e) {
    const privateReg = /^#mj私聊设置(cd|冷却)(\d{1,5})$/i
    const switchReg = /(关闭|开启)/
    const [privateCoolTime, switchCoolTime] = [
    ...
    if (switchCoolTime) {
      this.setPrivatePolicy(
        'private_cool_time_switch',
        switchCoolTime[1] === '开启'
      )
    }
    return true
  ...
  async setBlackGroup (e) {
    const setBlackReg = /^#mj封禁?群(\d{5,11})?$/i
    const unsetBlackReg = /^#mj解封群(\d{5,11})?$/i
    const [setBlack, unsetBlack] = [
      setBlackReg.exec(e.msg),
      unsetBlackReg.exec(e.msg)
    ]
    // 获取群号
    const groupNumber = e.msg.match(/群(\d{5,11})?/)[1] || e.group_id
    // 判断是拉黑
    if (setBlack) {
  ...
  async setBlackUser (e) {
    const setBlackReg = /^#mj拉黑(\d{5,11})?$/i
    const unsetBlackReg = /^#mj解(除拉)?黑(\d{5,11})?$/i
    const [setBlack, unsetBlack] = [
      setBlackReg.exec(e.msg),
      unsetBlackReg.exec(e.msg)
    ]
    // 获取QQ号
    const qqNumber = e.at || e.msg.match(/黑(\d{5,11})?/)[1]
    if (!qqNumber) {
  ...
  async setGroupPolicy (groupID, key, value) {
    ...
    if (groupID === 'global') {
      ...
      // 判断群号是否存在
      if (!policy.group_property[groupID]) policy.group_property[groupID] = {}
    ...
    try {
      ...
      if (groupID != 'global') {
        try {
          const groupInfo = await Bot.getGroupInfo(Number(groupID))
      ...
      const msg = [
        groupID === 'global'
          ...
          : groupID === this.e.group_id
            ? '本群的'
            : `群[${groupName}](${groupID})的`,
        key === 'group_cool_time' || key === 'group_cool_time_switch'
          ? '共享cd'
          : key === 'person_cool_time' || key === 'person_cool_time_switch'
            ? '个人cd'
            : '???',
        '已',
        key === 'group_cool_time' || key === 'person_cool_time'
          ? `设为${value}秒`
          : key === 'group_cool_time_switch' ||
            key === 'person_cool_time_switch'
            ? value
              ? '开启'
              : '关闭'
            : '???'
  ...
  async setPrivatePolicy (key, value) {
    ...
    try {
      ...
      const msg = [
        '私聊',
        key === 'private_cool_time' || key === 'private_cool_time_switch'
          ? '冷却时间'
          : '???',
        '已',
        key === 'private_cool_time'
          ? `设置为${value}秒`
          : key === 'private_cool_time_switch'
            ? value
              ? '开启'
              : '关闭'
            : '???'
