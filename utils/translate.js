...
async function BaiduTranslate(msg) {
    ...
    let sign = crypto.createHash('md5').update((await Config.getConfig()).baidu_translate.appid + msg + salt + (await Config.getConfig()).baidu_translate.appkey).digest('hex')
    let url = `${translateAPI}?q=${msg}&from=zh&to=en&appid=${(await Config.getConfig()).baidu_translate.appid}&salt=${salt}&sign=${sign}`
...
async function YoudaoTranslate(msg) {
    ...
    let sign = crypto.createHash('sha256').update((await Config.getConfig()).youdao_translate.appid + input + salt + curtime + (await Config.getConfig()).youdao_translate.appkey).digest('hex');
    let url = `${translateAPI}?appKey=${(await Config.getConfig()).youdao_translate.appid}&q=${msg}&from=auto&to=en&salt=${salt}&sign=${sign}&signType=v3&curtime=${curtime}`
    let res = await fetch(url)
    let json = await res.json()
    try {
        if (json.errorCode != 0) {
...
async function yenai_youdao(msg) {
    ...
    const sign = crypto.createHash('md5').update((payload.client + i + salt + key), 'utf-8').digest('hex');
